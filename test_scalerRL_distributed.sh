#!/bin/bash
# 분산 환경 테스트 스크립트: Trainer 1개, Orchestrator 1개, Sampler 2개
# 원격 서버(172.16.162.40)로 통신
...
ORCH_HOST="${ORCH_HOST:-172.16.162.40}"
...
# Run ID 생성
RUN_ID=$(date +"%Y%m%d_%H%M%S")_test_scalerRL_dist
LOG_DIR="logs/${RUN_ID}"
mkdir -p "$LOG_DIR"

echo "========================================="
echo "ScalerRL 분산 테스트 시작"
echo "Run ID: $RUN_ID"
...
for i in {1..30}; do
    ...
    if [ $i -eq 30 ]; then
        echo "경고: Orchestrator에 연결할 수 없습니다 ($ORCH_URL)"
        echo "Orchestrator가 원격 서버($ORCH_HOST)에서 실행 중인지 확인하세요"
        echo "계속 진행합니다..."
    fi
    sleep 1
done
